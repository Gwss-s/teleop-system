...
> 历史排查记录与基准数字见 `teleop_official_comparison.md` 与 `troubleshooting.md`。

## 1. 数据通路(哪一段延迟由哪个仪表负责)

```
[Pico Ultra 4 头显]
   | (1) WiFi 空口: 热点 hil-teleop/wlo1, 位姿流 90Hz  <- net_monitor(ping) 测这段
   v
...
| ~~V2 锚定/V3 橡皮筋~~ | 误判"遥操代码导致延迟"时期的改动 | **已删除(2026-08-26 定夺)**。真因=链路开太久整体退化,全链路重启后 V1 完全正常(详见 `teleop_official_comparison.md`) |
